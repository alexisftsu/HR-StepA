...
def parse_gamma_from_line(line):
    ...
    for t in toks:
        try:
            ...
            pass
    # tomar candidatos plausibles como gamma
    cand = [v for v in vals if 10.0 <= v <= 1.0e14]
    if not cand:
        return None
    # en líneas con índice y gamma, el menor >10 suele ser gamma
    return min(cand)

def load_gammas(folder, Tcap=None):
    seen=set()
    for name in os.listdir(folder):
        # saltar archivos de hash/metadata
        low = name.lower()
        if "hash" in low:
            continue
        ...
        with open(path,"r",encoding="utf-8",errors="ignore") as f:
            for line in f:
                ...
                if g is None: 
                    continue
                if Tcap is not None and g > Tcap: 
                    continue
                seen.add(round(g,12))
    return np.array(sorted(seen))

# ---- C0' y X0 a partir de ceros ----
...
# ---- Kernel: sup|h''| para hat_g(t)=(1-|t|)^3_+ ----
...
# ---- Verificación (opcional) de |zeta'/zeta| ----
T_min = math.sqrt(2.0)
K_eff = 10.0
K_max = None
if args.scan_zld:
...
if os.path.exists(args.vk):
    ...
    # aceptar tanto B_VK/b_VK como BVK/beta_VK
