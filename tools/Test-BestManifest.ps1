...
try { $m = Get-Content -Raw $Path | ConvertFrom-Json } catch { Write-Error "JSON inválido: $($_.Exception.Message)"; exit 1 }
$missing = @()
...
if ($missing.Count) { Write-Error "Manifest incompleto. Faltan: $($missing -join ', ')"; exit 1 } else { "Manifest OK" }
